...
public class FlexiPlannerUI{
	static private JLabel displayedMonth, displayedYear;
	static private JButton prevMonth, nextMonth;
	static private JTable calendar1;
	static private DefaultTableModel calendar2;
	static private JScrollPane calendarScroll;
	static private JPanel schedulerPanel;
	static private JComboBox selectYear;
	static private JFrame schedulerFrame;
	static private JTextField inputCommand;
	static private int actualYear, actualMonth, actualDay, currentDisplayedYear, currentDisplayedMonth;
	...
	public static void main (String args[]){
		JFrame.setDefaultLookAndFeelDecorated(false);//the frame is changed to different style here
		schedulerFrame = new JFrame("FlexiPlanner");//create new frame named *CALENDAR*                
		schedulerFrame.setUndecorated(false);//the frame is changed
		schedulerFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);//Program exits when closed
		schedulerFrame.setSize(700,410);//set frame size
		schedulerFrame.setResizable(false);
		schedulerFrame.setVisible(true);

		calendar2 = new DefaultTableModel(){public boolean isCellEditable(int rowIndex, int mColIndex){return false;}};//get dafault table model for calendar
		calendar1 = new JTable(calendar2);//create new table
		calendarScroll = new JScrollPane(calendar1);
		schedulerPanel = new JPanel(null);//this is needed in order to show table

		schedulerFrame.getContentPane().add(schedulerPanel);//add panel to frame(add table to frame)
		...
		displayedMonth.setBounds(160-displayedMonth.getPreferredSize().width/2, 5, 100, 25);
		...
		showContent.setText(LogicTest.getData());
		...
		for (int i=0; i<7; i++){
			calendar2.addColumn(headers[i]);//add it on top column(left to right)
		}
	...
	private static void executeKeyAction(final JLabel commandFeedback) {
	...
	private static void refreshCalendar(int month, int year){
		...
		if (month == 0 && year == actualYear){prevMonth.setEnabled(false);} //disable button as already pass
		if (month == 11 && year >= actualYear+20){nextMonth.setEnabled(false);} //disable button as out of range
		displayedMonth.setText(months[month]); //Refresh the month label (at the top)
		displayedMonth.setBounds(160-displayedMonth.getPreferredSize().width/2, 5, 180, 25); //Re-align label with calendar
		selectYear.setSelectedItem(String.valueOf(year)); //Select the correct year in the combo box
		...
		calendar1.setDefaultRenderer(calendar1.getColumnClass(0), new Calendar1Renderer());//using Calendar1Renderer class to set table display
	}//end of refreshCalendar method

	//class Calendar1Renderer used for editting how things are displayed
	static class Calendar1Renderer extends DefaultTableCellRenderer{
	...
	static class Years_Action implements ActionListener{
		public void actionPerformed (ActionEvent e){
