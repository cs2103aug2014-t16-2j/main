...
public class FlexiPlannerUI implements HotKeyListener {
	...
	public void loadInterfaceandData() throws IOException, ParseException {
		...
		showUserExecutedCommandLabel.setFont(new Font("Times New Roman",
				Font.BOLD, 15));
		showUserExecutedCommandLabel.setForeground(Color.BLUE);
		showUserExecutedCommandLabel.setBorder(BorderFactory
				.createCompoundBorder(border,
						BorderFactory.createEmptyBorder(5, 5, 5, 5)));
		showUserExecutedCommandLabel.setText("Executed Commands");
		showUserExecutedCommand = new JTextArea();
		showUserExecutedCommand.setFont(new Font("Times New Roman", Font.BOLD,
				14));
		...
		showUserExecutedCommandScroll = new JScrollPane(
				showUserExecutedCommand, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
				JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
		showUserExecutedCommandScroll
				.setColumnHeaderView(showUserExecutedCommandLabel);
		showUserExecutedCommandCollapsePane = new JXCollapsiblePane();
		showUserExecutedCommandCollapsePane
				.setContentPane(showUserExecutedCommandScroll);
		showUserExecutedCommandCollapsePane.setBounds(320, 4, 570, 0);
		showUserExecutedCommandCollapsePane.setPreferredSize(new Dimension(570,
				495));
		...
		executeKeyAction(commandFeedback, showOverDueTask, showTodayTask,
				showUserExecutedCommand, showCategory, showOverDueTaskScroll,
				showTodayTaskScroll, showUserExecutedCommandScroll,
				showCategoryScroll, showOverDueCollapsePane, todayCollapsePane,
				showUserExecutedCommandCollapsePane);
	}

	private void executeKeyAction(final JTextArea commandFeedback,
			final JTextArea showOverDueTask, final JTextArea showTodayTask,
			final JTextArea showUserExecutedCommand,
			final JTextArea showCategory,
			final JScrollPane showOverDueTaskScroll,
			final JScrollPane showTodayTaskScroll,
			final JScrollPane showUserExecutedCommandScroll,
			final JScrollPane showCategoryScroll,
			final JXCollapsiblePane overDueCollapsePane,
			final JXCollapsiblePane todayCollapsePane,
			final JXCollapsiblePane showUserExecutedCommandCollapsePane) {
		inputCommand.addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent e) {
				int key = e.getKeyCode();
				int overDueScrollPane = showOverDueTaskScroll
						.getVerticalScrollBar().getModel().getValue();
				int valueTodayScrollPane = showTodayTaskScroll
						.getVerticalScrollBar().getModel().getValue();
				int valueCustomTextArea = showUserExecutedCommandScroll
						.getVerticalScrollBar().getModel().getValue();
				...
				case KeyEvent.VK_ENTER:
					...
					try {
						showUserExecutedCommand.setText(logic
								.getData(userCommand));
					} catch (IOException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
				...
				case KeyEvent.VK_F4:
					...
					*/
					...
					break;
				case KeyEvent.VK_F5:
					showOverDueTaskScroll.getVerticalScrollBar().getModel()
							.setValue(overDueScrollPane - 5);
					break;
				case KeyEvent.VK_F6:
					showOverDueTaskScroll.getVerticalScrollBar().getModel()
							.setValue(overDueScrollPane + 5);
					break;
				case KeyEvent.VK_F7:
					showTodayTaskScroll.getVerticalScrollBar().getModel()
							.setValue(valueTodayScrollPane - 5);
					break;
				case KeyEvent.VK_F8:
					showTodayTaskScroll.getVerticalScrollBar().getModel()
							.setValue(valueTodayScrollPane + 5);
					break;
				case KeyEvent.VK_F9:
					showUserExecutedCommandScroll.getVerticalScrollBar()
